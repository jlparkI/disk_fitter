...
def gen_plot(qtapp, data_type='disk'):
  ...
  try:
    ...
    #limits are below.
    if float(current_model.miccutoffS) < 0.06 or float(current_model.miccutoffS) > 120:
      return "You have entered an invalid MIC breakpoint (<0.06 or >120). Try again."
    if float(current_model.miccutoffR) < 0.06 or float(current_model.miccutoffR) > 120:
      return "You have entered an invalid MIC breakpoint (<0.06 or >120). Try again."
    if float(current_model.diskcutoffS) < 0.12 or float(current_model.diskcutoffS) > 64:
      return "You have entered an invalid disk cutoff(<0.12 or >64). Try again."
    if float(current_model.diskcutoffR) < 0.12 or float(current_model.diskcutoffR) > 64:
      return "You have entered an invalid disk cutoff(<0.12 or >64). Try again."
  except:
  ...
  try:
    yreal = np.log(np.clip(np.asarray(current_model.current_dataset['mics'].values), a_min=0.016, a_max = 256))
    if data_type == 'mic':
      x = np.log(np.clip(np.asarray(current_model.current_dataset['disks'].values), a_min=0.016, a_max=256))
    else:
      x = np.clip(np.asarray(current_model.current_dataset['disks'].values), a_min=5, a_max=50)
  except:
  ...
  error_code = current_model.update_error_tables(data_type)
  if error_code != '0':
    return error_code
  ...
  mic_cutoffS = float(current_model.miccutoffS)
  ...
  if data_type == 'mic':
    ...
    if current_model.diskcutoffS <= 128:
      rounded_cutoffS = xbins[xbins.index(current_model.diskcutoffS)+1]
    else:
      rounded_cutoffS = 256
    ax.plot(np.full(vertpoints1.shape[0], np.log(rounded_cutoffS)),
            np.log(vertpoints1), color='k', linewidth=0.5)
    ax.plot(np.full(vertpoints1.shape[0], np.log(current_model.diskcutoffR)), np.log(vertpoints1), color='k', linewidth=0.5)
    ...
    ax.plot(np.full(vertpoints1.shape[0], current_model.diskcutoffS), np.log(vertpoints1), color='k', linewidth=0.5)
    ax.plot(np.full(vertpoints1.shape[0], current_model.diskcutoffR+1), np.log(vertpoints1), color='k', linewidth=0.5)
  ...
  try:
    ...
    ax.plot(horizpoints1, np.full(horizpoints1.shape[0], np.log(float(current_model.miccutoffR))), color='k', linewidth=0.5)
  ...
  ax2.axis('off')
  generate_tabletext.fix_table(table)
  qtapp.canvas.draw()
  return '0'
