import numpy as np, matplotlib, traceback
...
def gen_plot(qtapp, data_type='disk'):
	try:
		test_vals = [float(current_value) for current_value in [qtapp.miccutoffS, qtapp.miccutoffR,
									qtapp.diskcutoffS, qtapp.diskcutoffR]]
		if test_vals[0] < 0.06 or test_vals[0] > 120:
			return "You have entered an invalid MIC breakpoint (<0.06 or >120). Try again."
		if test_vals[1] < 0.06 or test_vals[1] > 120:
			return "You have entered an invalid MIC breakpoint (<0.06 or >120). Try again."
		if test_vals[2] < 0.01 or test_vals[2] > 64:
			return "You have entered an invalid disk cutoff(<1 or >50). Try again."
		if test_vals[3] < 0.01 or test_vals[3] > 64:
			return "You have entered an invalid disk cutoff(<1 or >50). Try again."
	except:
		traceback.print_exc()
	...
	try:
		...
		if data_type == 'mic':
			...
			ax.xaxis.tick_top()
			ax.plot(np.full(vertpoints1.shape[0], np.log(float(qtapp.diskcutoffS))), np.log(vertpoints1), color='k', linewidth=0.5)
			ax.plot(np.full(vertpoints1.shape[0], np.log(float(qtapp.diskcutoffR))), np.log(vertpoints1), color='k', linewidth=0.5)
